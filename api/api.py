...
@try_return_none
def get_all_subcards_by_user_id(user_id):
    """
    Получает все субкарты пользователя.
    Аргумент: user_id.
    Возвращает список (возможно пустой) строк из БД (кортежей), None при ошибке.
    """
    return DB.fetch_all("""
        SELECT s.id, s.card_id, s.category_id, s.amount, s.description, s.is_active
        FROM card c JOIN subcard s ON s.card_id = c.id
        WHERE c.owner_id = %(user_id)s;
